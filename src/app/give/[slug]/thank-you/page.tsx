...
export async function generateMetadata({
  ...
  return { title: `Thank you — ${church.name}` };
}
...
export default async function GiveThankYouPage({
  ...
  const church = await db.church.findUnique({
    where: { slug },
    select: { name: true, slug: true, isDemo: true, logoUrl: true, accentColor: true },
  });
    ...
    <div className="grid min-h-screen place-items-center bg-[#faf8f4] px-4">
      <div className="w-full max-w-md rounded-2xl border border-[#e8e2d6] bg-white p-8 text-center shadow-sm">
        ...
        <p className="mt-3 text-sm text-[#6b6560]">
          {church.name} is blessed by your generosity. A receipt is on its way to you. May God bless you abundantly!
        </p>
        {ref && (
          <div className="mt-6 rounded-xl border border-[#e8e2d6] bg-[#faf8f4] p-4 text-xs text-[#a09888]">
            Reference: <span className="font-mono text-[#6b6560]">{ref}</span>
          </div>
        )}
        <a
          href={`/give/${church.slug}`}
          className="mt-6 inline-block text-sm font-medium"
          style={{ color: church.accentColor }}
        >
          Give again
        </a>
      </div>
