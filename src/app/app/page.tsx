...
export default async function DashboardPage() {
  ...
  return (
    <div>
      <PageHeader title={`${greeting}, ${session.name.split(" ")[0]} 👋`} description={`Here's what's happening at ${session.branch} today.`}>
        <SyncStatus />
        <Button size="sm">
          <Plus /> Quick add
        </Button>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Active members" value={kpis.activeMembers} change={kpis.membersChange} icon={Users} />
        <StatCard label="Weekly attendance" value={kpis.weeklyAttendance} change={kpis.attendanceChange} icon={CalendarCheck2} />
        <StatCard label="Monthly giving" value={kpis.monthlyGiving} prefix="₵" change={kpis.givingChange} icon={HandCoins} />
        <StatCard label="Message reach" value={kpis.messageReach} change={kpis.reachChange} icon={MessageSquare} />
      </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        {quickActions.map((a) => (
          <Link key={a.label} href={a.href}>
            <div className="flex items-center gap-3 rounded-2xl border border-line bg-surface/40 p-4 transition-all hover:-translate-y-0.5 hover:border-primary/30 hover:bg-surface">
              <span className="grid size-10 place-items-center rounded-xl bg-primary/10 text-primary-bright">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <div className="flex items-center justify-between p-5 pb-0">
            ...
            <Badge variant="success">+{kpis.givingChange}%</Badge>
          </div>
          <div className="p-3">
            <TrendAreaChart data={givingTrend} />
          </div>
        </Card>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-3">
        ...
        <Card>
          <div className="flex items-center justify-between p-5 pb-3">
            <h3 className="flex items-center gap-2 font-display text-lg font-semibold">
              <Cake className="size-5 text-gold" /> Today&rsquo;s celebrations
            </h3>
          </div>
          <div className="space-y-1 px-3 pb-3">
            {todaysBirthdays.map((p) => (
              <div key={p.id} className="flex items-center justify-between rounded-xl px-2 py-2 hover:bg-surface-2">
                ...
                <Button size="sm" variant="ghost" className="text-gold">
                  <Send className="size-3.5" /> Bless
                </Button>
              </div>
            ))}
            <div className="mx-2 mt-2 rounded-xl bg-gold/10 px-3 py-2 text-xs text-gold">
              Birthday SMS blessings are sent automatically at 7:00 AM.
            </div>
          </div>
        </Card>
        ...
        <Card>
          <div className="flex items-center justify-between p-5 pb-3">
            ...
            <Link href="/app/events" className="text-xs text-primary-bright hover:underline">
              View all
            </Link>
          </div>
          <div className="space-y-1 px-3 pb-3">
            {events.slice(0, 4).map((e) => (
              <div key={e.id} className="flex items-center gap-3 rounded-xl px-2 py-2 hover:bg-surface-2">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs text-ink-faint">{e.time} · {e.branch}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <Card>
          <div className="flex items-center justify-between p-5 pb-3">
            <h3 className="flex items-center gap-2 font-display text-lg font-semibold">
              <Heart className="size-5 text-danger" /> Care & follow-up
            </h3>
          </div>
          <div className="space-y-1 px-3 pb-3">
            {careTasks.map((t) => (
              <div key={t.id} className="flex items-start gap-3 rounded-xl px-2 py-2 hover:bg-surface-2">
                <span
                  className={`mt-1.5 size-2 shrink-0 rounded-full ${
                    t.priority === "high" ? "bg-danger" : t.priority === "medium" ? "bg-warning" : "bg-ink-faint"
                  }`}
                />
                <div className="min-w-0 flex-1">
      ...
      <div className="mt-6 flex justify-center">
        <Link href="/app/people">
          <Button variant="ghost">
            Explore your congregation <ArrowRight className="size-4" />
          </Button>
        </Link>
      </div>
    </div>
