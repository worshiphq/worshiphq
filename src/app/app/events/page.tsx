  ...
  return (
    <div>
      <PageHeader title="Events & calendar" description="Services, seminars and camps — with registration and QR check-in.">
        ...
        <Button size="sm"><Plus /> Create event</Button>
      </PageHeader>

      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Upcoming events" value={events.length} icon={CalendarDays} />
        <StatCard label="Total registered" value={totalRegistered} change={9.2} icon={Users} />
        <StatCard label="Paid events" value={events.filter((e) => e.paid).length} icon={Ticket} />
        <StatCard label="Ticket revenue" value={48650} prefix="₵" change={14.5} icon={Ticket} />
      </div>
          ...
          return (
            <Card key={e.id} hover className="flex flex-col p-5">
              <div className="mb-4 flex items-start justify-between">
                ...
                {e.paid ? (
                  <Badge variant="gold">{formatCurrency(e.price ?? 0)}</Badge>
                ) : (
              ...
              {e.registered > 0 && (
                <div className="mt-4">
                  <div className="mb-1.5 flex items-center justify-between text-xs">
                    <span className="text-ink-muted">{e.registered} registered</span>
                    <span className="text-ink-faint">{pct}% full</span>
                  </div>
                  <div className="h-2 overflow-hidden rounded-full bg-surface-2">
                    <div className="h-full rounded-full bg-gradient-to-r from-primary to-primary-bright" style={{ width: `${pct}%` }} />
                  </div>
                </div>
              )}
