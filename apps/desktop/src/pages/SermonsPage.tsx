...
import {
  Plus, Loader2, BookOpen, Trash2, Search, Mic, Pencil,
} from "lucide-react";
...
function SermonForm({ churchId, existing, onClose, onSaved }: { churchId: string; existing?: any; onClose: () => void; onSaved: () => void }) {
  ...
  const [form, setForm] = useState({
    ...
    date: existing?.date || new Date().toISOString().slice(0, 10), notes: existing?.notes || "",
  });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (existing) {
      ...
      await db.insert("sermon", { id: uuid(), church_id: churchId, ...data });
